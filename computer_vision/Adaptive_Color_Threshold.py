...
while True:
    ...
    if rectangles:
        for rect in rectangles:
            # 绘制矩形轮廓
            #cv2.drawContours(img_contour, [rect], -1, (255, 255, 255), 1)
        ...
        if green_laser:
            ...
            cv2.putText(img_contour, f"green:{green_laser}", (20, 130),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 1, cv2.LINE_AA)
            ser.write(f"green={green_laser}!".encode())
        if red_laser and green_laser:

            distance = np.linalg.norm(np.array(red_laser) - np.array(green_laser))
            ser.write(f"#P1={distance}!".encode())

        if red_laser and is_inside_polygon(red_laser, rect):
            print("Rinside")
        else :
            print("Routside")
        if green_laser and is_inside_polygon(green_laser, rect):
            print("Ginside")
        else :
            print("Goutside")
